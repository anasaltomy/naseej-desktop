...
export function registerCategoryHandlers(): void {
  ...
  ipcMain.handle("categories:getAll", () => {
    const cats = db()
      .prepare("SELECT * FROM categories ORDER BY parent_id NULLS FIRST, name")
      .all() as Row[];
    const sizes = db()
      .prepare(
  ...
  ipcMain.handle(
    ...
    (
      ...
      if (data.hasStandardSizes !== undefined) {
        fields.push("has_standard_sizes = ?");
        values.push(data.hasStandardSizes ? 1 : 0);
      }
      db().transaction(() => {
        if (fields.length > 0) {
          values.push(id);
        ...
        if (data.sizeIds !== undefined) {
          db()
            .prepare("DELETE FROM category_sizes WHERE category_id = ?")
            .run(id);
          const insertSize = db().prepare(
            "INSERT INTO category_sizes (category_id, size_id) VALUES (?, ?)",
          );
