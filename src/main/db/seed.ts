...
function seedMerchantConfig(db: Database.Database): void {
  ...
  const insert = db.prepare(
    "INSERT INTO merchant_config (key, value) VALUES (?, ?)",
  );
  const configs = [
    ["name", "Naseej"],
...
function seedStaff(db: Database.Database): void {
  ...
  const seedAll = db.transaction(() => {
    insert.run("u1", "Anas", "Al-Toumi", "anas@naseej.sa", "admin", bcrypt.hashSync("1234", SALT_ROUNDS));
    insert.run("u2", "Sara", "Al-Otaibi", "sara@naseej.sa", "cashier", bcrypt.hashSync("5678", SALT_ROUNDS));
    insert.run(
      "u3",
      "Khalid",
      "Al-Shammari",
      "khalid@naseej.sa",
      "senior_cashier",
      bcrypt.hashSync("9012", SALT_ROUNDS),
    );
  });
  seedAll();
}
...
function seedBrands(db: Database.Database): void {
  ...
  const seedAll = db.transaction(() => {
    for (const [id, name] of data) insert.run(id, name);
  });
  seedAll();
}
...
function seedColors(db: Database.Database): void {
  ...
  const data = [
    ...
    ["c3", "Navy", "#1E3A5F"],
    ["c4", "Red", "#DC2626"],
    ["c5", "Forest Green", "#166534"],
    ["c6", "Camel", "#C19A6B"],
    ["c7", "Burgundy", "#800020"],
    ["c8", "Sky Blue", "#87CEEB"],
    ["c9", "Charcoal", "#36454F"],
    ["c10", "Olive", "#808000"],
    ["c11", "Blush Pink", "#DE5D83"],
    ["c12", "Sand", "#C2B280"],
    ["c13", "Beige", "#C8B89A"],
  ];
  const seedAll = db.transaction(() => {
    for (const [id, name, hex] of data) insert.run(id, name, hex);
  });
  seedAll();
}
...
function seedSizes(db: Database.Database): void {
  ...
  const seedAll = db.transaction(() => {
    for (const [id, name, ord] of data) insert.run(id, name, ord);
  });
  seedAll();
}
...
function seedCategories(db: Database.Database): void {
  ...
  const seedAll = db.transaction(() => {
    ...
    insertCatSize.run("cat-mens", "s2");
    insertCatSize.run("cat-mens", "s3");
    insertCatSize.run("cat-mens", "s4");
    insertCatSize.run("cat-mens", "s5");
    insertCatSize.run("cat-mens", "s6");

    insertCat.run("cat-womens", "Women", "women", null, 1);
    insertCatSize.run("cat-womens", "s1");
    insertCatSize.run("cat-womens", "s2");
    insertCatSize.run("cat-womens", "s3");
    insertCatSize.run("cat-womens", "s4");
    insertCatSize.run("cat-womens", "s5");

    insertCat.run("cat-shirts", "Shirts", "shirts", "cat-mens", 0);
    insertCat.run("cat-pants", "Pants", "pants", "cat-mens", 1);
    insertCatSize.run("cat-pants", "s7");
    insertCatSize.run("cat-pants", "s8");
    insertCatSize.run("cat-pants", "s9");
    insertCatSize.run("cat-pants", "s10");
    ...
    insertCatSize.run("cat-footwear", "s15");
    insertCatSize.run("cat-footwear", "s16");
    insertCatSize.run("cat-footwear", "s17");
    insertCatSize.run("cat-footwear", "s18");
    insertCatSize.run("cat-footwear", "s19");

    insertCat.run("cat-abayas", "Abayas", "abayas", "cat-womens", 1);
    insertCatSize.run("cat-abayas", "s1");
    insertCatSize.run("cat-abayas", "s2");
    insertCatSize.run("cat-abayas", "s3");
    insertCatSize.run("cat-abayas", "s4");
  });
  seedAll();
}
...
function seedRoles(db: Database.Database): void {
  ...
  const seedAll = db.transaction(() => {
    ...
    insert.run(
      "r2",
      "Store Manager",
      "Manages daily operations and staff",
      JSON.stringify([
        "pos:sell",
        "pos:refund",
        "pos:end_of_day",
    ...
    insert.run(
      ...
      JSON.stringify(["pos:sell", "catalog:view", "inventory:view"]),
      4,
    );
  });
  seedAll();
