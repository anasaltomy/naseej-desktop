...
import StockAdjustmentModal from "../windows/StockAdjustmentModal";
import CreateProductModal from "../windows/CreateProductModal";
import ProductDetailModal from "../windows/ProductDetailModal";
import EditProductModal from "../windows/EditProductModal";
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 flex-1 mx-2 max-w-sm">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={t(
                "features.catalog.screens.inventory.searchPlaceholder",
              )}
      ...
      {showFilters && (
        <div className="bg-card border-b border-border px-4 py-2 flex flex-wrap items-center gap-2 shrink-0">
          <select
            ...
            className="pos-input text-xs w-36 h-9"
          ...
          <select
            ...
            className="pos-input text-xs w-32 h-9"
      ...
      <div className="flex gap-px bg-border shrink-0">
        {[
          {
            label: t("features.catalog.screens.inventory.totalSKUs"),
            value: inventory.length,
            color: "text-foreground",
          },
          {
            label: t("features.catalog.screens.inventory.lowStock"),
            value: inventory.filter(isLowStock).length,
            color: "text-warning",
          },
          {
            label: t("features.catalog.screens.inventory.outOfStock"),
            value: inventory.filter(isOutOfStock).length,
            ...
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-sm font-medium text-foreground">
                  {t("features.catalog.screens.inventory.noVariantsFound")}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  {t("features.catalog.screens.inventory.tryDifferentSearch")}
                </p>
              </div>
            ) : (
              filtered.map((item) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={cn(
                        "grid grid-cols-[2fr_1fr_1fr_1fr_auto] px-4 py-3 items-center hover:bg-muted/50 transition-colors cursor-pointer",
                        outOfStock && "opacity-60",
                      )}
                      onClick={() => toggleExpanded(item.variantId)}
                      role="button"
                      tabIndex={0}
                      onKeyDown={(e) =>
                        e.key === "Enter" && toggleExpanded(item.variantId)
                      }
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {outOfStock ? (
                          <span className="badge-destructive">Out</span>
                        ) : lowStock ? (
          ...
          <div className="p-4 space-y-3">
            {groupedProducts.length === 0 ? (
              ...
              groupedProducts.map((group) => {
                ...
                const brandName = dbBrands.find(
                  (b) => b.id === group.brandId,
                )?.name;
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      tabIndex={0}
                      onKeyDown={(e) =>
                        e.key === "Enter" && toggleExpanded(group.productId)
                      }
                      aria-expanded={isExpanded}
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setDetailProductId(group.productId);
                            }}
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        {hasOut && (
                          <span className="badge-destructive">Has Out</span>
                        )}
                        {!hasOut && hasLow && (
                          <span className="badge-warning">Has Low</span>
                        )}
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            aria-label="Edit product"
                            onClick={(e) =>
                              handleEditClick(group.variants[0], e)
                            }
                            disabled={loadingEditId === group.productId}
                          >
                            {loadingEditId === group.productId ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            ) : (
                              <Pencil className="w-3.5 h-3.5" />
                            )}
                      ...
                      <div className="border-t border-border/50">
                        {group.variants.map((v) => {
                          ...
                          return (
                            <div
                              key={v.variantId}
                              className="grid grid-cols-[1fr_1.2fr_auto_auto_auto] px-4 py-2.5 border-b border-border/30 last:border-0 items-center gap-3 bg-muted/10"
                            >
