...
export default function CreateWarehouseModal({
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      await window.api?.locations.create({
        name: name.trim(),
        city: location.trim(),
      ...
      setErrors({ form: "Failed to create warehouse. Please try again." });
    } finally {
      setIsSaving(false);
    }
  }, [name, location, address, phone, onSuccess, handleClose]);
  ...
  return (
    <Modal
      ...
      description="Add a new inventory storage location"
      size="md"
      footer={footer}
    >
